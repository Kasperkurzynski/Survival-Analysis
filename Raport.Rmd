...
Podobna ależność występuje w przypadku związku ilości miedzi w moczu, a występowaniem powiększenia wątroby. Analogicznie jest również w przypadku zależności pomiędzy stężeniem barwnika żółciowego, a występowaniem powiększenia wątroby.
```{r fig.height=10, fig.width=8}
grid.arrange(z2,x2)
```
...
Średnie stężenie trójglicerydów we krwi rośnie również wraz ze stadium rozwoju choroby, choć w średnia w stadium III jest ona nieco niższa niż w stadium IV.
```{r}
tab6 <- daneKNN %>%
  group_by(stage) %>%
  summarise(N = n(), Mean = round(mean(trig),2), Min = min(trig), Max = max(trig), Sd = round(sd(trig),2))
...
* INTERPRETACJA PARAMETROW: 
  + jeżeli wiek wzrośnie o rok, hazard (ryzyko) zgonu rośnie o **3,3%** u osób bez hepatomegalii lub powiększenia wątroby, ceteris paribus
  + pacjenci z obecnościa hepatomegalii lub powiększenia wątroby(hepato1) mają hazard (ryzyko) zgonu o **66,2%** wyższy niż pacjenci z hepato = 0, ceteris paribus
  ...
  + wraz ze wzrostem ast o 1, hazard (ryzyko) zgonu rośnie o **0,4%**, ceteris paribus
  + wraz ze wzrostem protime o 1, hazard (ryzyko) zgonu rośnie o **38,0%**, ceteris paribus
...
* INTERPRETACJA PARAMETROW: 
  + jeżeli wiek wzrośnie o rok, hazard (ryzyko) zgonu rośnie o **3,4%** u osób bez hepatomegalii lub powiększenia wątroby, ceteris paribus (przy pozostałych wartościach zmiennych takich samych), ceteris paribus
  + pacjenci z obecnościa hepatomegalii lub powiększenia wątroby (hepato1) mają hazard (ryzyko) zgonu o **102%** wyższy niz pacjenci z hepato = 0, ceteris paribus
  + wraz ze wzrostem albumin o 1, hazard (ryzyko) zgonu maleje o **70%**, ceteris paribus
  + wraz ze wzrostem copper o 1, hazard (ryzyko) zgonu rośnie o **0,5%**, ceteris paribus
  + wraz ze wzrostem ast o 1, hazard (ryzyko) zgonu rośnie o **0,5%**, ceteris paribus
...
Ocenę tego, czy założenie proporcjonalności jest spełnione dokonuje się wzrokowo, sprawdzając czy punkty na wykresie układają się wzdłuż linii poziomej. Wyraźna obserwowana tendencja w przebiegu tej krzywej, sugeruje brak proporcjonalności zmiennej.
...
* INTERPRETACJA PARAMETROW: 
  + jeżeli wiek wzrośnie o 1 rok, hazard (ryzyko) zgonu rośnie o **4,9%** u osób bez hepatomegalii lub powiększenia wątroby, ceteris paribus 
  + pacjenci z obecnością hepatomegalii lub powiększenia wątroby(hepato1) mają hazard (ryzyko) zgonu o **100,5%** wyższy niż pacjenci z hepato = 0 ceteris paribus
  + wraz ze wzrostem albumin o 1, hazard (ryzyko) zgonu maleje o **76%**
  + wraz ze wzrostem copper o 1, hazard (ryzyko) zgonu rośnie o **0,6%** ceteris paribus
  + wraz ze wzrostem ast o 1, hazard (ryzyko) zgonu rośnie o **0,8%** ceteris paribus
...
Jeżeli zmienna objaśniająca 𝑋 jest ilościowa, to wprowadzenie jej do modelu Coxa jest równoważne przyjęciu założenia, że łączy ją związek liniowy z logarytmem hazardu. Jedna z najcześciej spotykanych metod graficznych jest metoda reszt martyngalowych. Polega na wyznaczeniu reszt martyngałowych dla modelu bez zmiennych objaśniających i zestawieniu ich z badaną zmienną objaśniającą na wykresie korelacyjnym
...
* jezeli wiek wzrosnie o 1 rok, hazard (ryzyko) zgonu rośnie o **5,0%** u osób bez hepatomegalii lub powiększenia wątroby, ceteris paribus 
* pacjenci z obecnościa hepatomegalii lub powiększenia wątroby(hepato1) mają hazard (ryzyko) zgonu o **100,5%** wyzszy niz pacjenci z hepato = 0 ceteris paribus
* wraz ze wzrostem albumin o 1, hazard (ryzyko) zgonu maleje o **76%**, ceteris paribus
* wraz ze wzrostem copper o 1, hazard (ryzyko) zgonu rośnie o **0,6%** ceteris paribus
* wraz ze wzrostem ast o 1, hazard (ryzyko) zgonu rośnie o **0,8%** ceteris paribus
...
Porównując miary dopasowania, najlepszy okazał się model z istotnymi zmiennymi oraz model po usunięciu jednostek wpływowych. Miara tych modeli jest taka sama. Niższą wartość przyjętej miary posiada model ze zmiennymi, które potwierdzały założenie o proporcjonalności hazardu i został oszacowany na wszystkich 312 jednostkach.
```{r}
